...
def get_conditional_probability(data, query, condititions):
    ...
    # =========================================================
    p = (numpy.absolute(get_data_frequency(data, combined_dictionary)))/(numpy.absolute(get_data_frequency(data, condititions)) + epsilon)
    # =========================================================
    return p
...
def exhaustive_enum(G, X, E, e):
    ...
    for assignment in assignments:
        ...
        for node in G.nodes:
            ...
            # =========================================================
            product = evaluate_node(G, node.data_id, combined_dictionary[node.data_id], combined_dictionary)
            tmp = tmp * product
            # =========================================================
        total = total + tmp
    return total
...
def sample(G, X, E, e):
    ...
    for variable in X:
        parents = dict(zip(variables, assignment))
            ...
            #
            #  - Use G.get_node_probability(query, parents) to get P(query | parents(query)) (KAYTA EVALUATE NODE!!)
